files <- Sys.glob(file.path("/media/yuhua/yuhua_projects/enhProj/ENHData/htseqcountfile_XW","*","htseq_count.txt"))
...
sampleid <- substring(files[1],posIndex+18,posIndex+attr(posIndex,'match.length')-13)
...
for (filename in files[2:length(files)]){
	if(file.info(filename)[1] > 0){
		...
		sampleid <- substring(filename,posIndex+18,posIndex+attr(posIndex,'match.length')-13)
		cat(sampleid,"\n")
		columnNames <- append(columnNames,sampleid)
	}
}
colnames(expData) <- columnNames
write.table(expData,file="/media/yuhua/yuhua_projects/enhProj/ENHData/enhancer_XW_counts.txt",sep='\t',quote=FALSE,row.names=T,col.names=T) 
